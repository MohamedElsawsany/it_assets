...
from .forms import MaintenanceForm, AccessoryMaintenanceForm, CloseMaintenanceForm
...
@login_required
@permission_required(Perms.MAINTENANCE_VIEW)
def acc_maintenance_data(request):
    search   = request.GET.get('search', '').strip()
    status_f = request.GET.get('status', '').strip()
    type_f   = request.GET.get('type', '').strip()
    show_cost = has_permission(request.user, Perms.MAINTENANCE_VIEW_COST)
    ...
    for r in page_obj:
        acc = r.accessory
        label = acc.accessory_type.name
        if acc.brand:
            label += f' ({acc.brand.name})'
        label += f' — {acc.serial_number}' if acc.serial_number else ' — (No S/N)'
        item = {
            'id': r.pk,
            'accessory_id': r.accessory_id,
            'accessory_label': label,
            'maintenance_type': r.maintenance_type,
            'maintenance_type_display': r.get_maintenance_type_display(),
            'vendor_name': r.vendor_name or '',
            'issue_description': r.issue_description[:80],
...
@login_required
def acc_maintenance_detail(request, pk):
    if not has_permission(request.user, Perms.MAINTENANCE_VIEW):
        return JsonResponse({'success': False, 'message': _('Permission denied.')}, status=403)
    show_cost = has_permission(request.user, Perms.MAINTENANCE_VIEW_COST)
    rec = get_object_or_404(
        AccessoryMaintenanceRecord.objects.filter(
            accessory__site__in=request.user.get_allowed_sites()
        ).select_related('accessory', 'accessory__accessory_type', 'accessory__brand', 'created_by', 'updated_by'),
        pk=pk,
    )
    acc = rec.accessory
    label = acc.accessory_type.name
...
@login_required
@require_http_methods(['POST'])
def acc_maintenance_create(request):
    if not has_permission(request.user, Perms.MAINTENANCE_CREATE):
        return JsonResponse({'success': False, 'message': _('Permission denied.')}, status=403)
    form = AccessoryMaintenanceForm(request.POST)
    if form.is_valid():
        rec = form.save(commit=False)
        rec.created_by = request.user
        accessory = rec.accessory
        if accessory.maintenance_records.filter(returned_date__isnull=True).exists():
            return JsonResponse({
                'success': False,
                'message': _('This accessory already has an open maintenance record. Please close it before creating a new one.'),
            })
        rec.previous_flag = accessory.flag
        rec.save()
        accessory.flag = DeviceFlag.UNDER_MAINTENANCE
        accessory.save()
        return JsonResponse({'success': True, 'message': _('Maintenance record created successfully.')})
    errors = {f: [str(e) for e in v] for f, v in form.errors.items()}
    return JsonResponse({'success': False, 'errors': errors})
...
@login_required
@require_http_methods(['POST'])
def acc_maintenance_edit(request, pk):
    if not has_permission(request.user, Perms.MAINTENANCE_EDIT):
        return JsonResponse({'success': False, 'message': _('Permission denied.')}, status=403)
    rec = get_object_or_404(AccessoryMaintenanceRecord, pk=pk)
    form = AccessoryMaintenanceForm(request.POST, instance=rec)
    if form.is_valid():
        obj = form.save(commit=False)
        obj.updated_by = request.user
        obj.save()
...
def _restore_accessory_flag(accessory, previous_flag):
    """Restore accessory flag after closing/deleting the last open maintenance record."""
    if previous_flag and previous_flag != DeviceFlag.UNDER_MAINTENANCE:
        accessory.flag = previous_flag
    else:
        has_active = accessory.accessory_assignments.filter(returned_date__isnull=True).exists()
        accessory.flag = DeviceFlag.ASSIGNED if has_active else DeviceFlag.AVAILABLE
    accessory.save()


@login_required
@require_http_methods(['POST'])
def acc_maintenance_close(request, pk):
    if not has_permission(request.user, Perms.MAINTENANCE_CLOSE):
        return JsonResponse({'success': False, 'message': _('Permission denied.')}, status=403)
    rec = get_object_or_404(AccessoryMaintenanceRecord, pk=pk)
    if not rec.is_open:
    ...
    if form.is_valid():
        ...
        rec.updated_by = request.user
        rec.save()
        other_open = rec.accessory.maintenance_records.filter(returned_date__isnull=True).exclude(pk=rec.pk).exists()
        if not other_open:
...
@login_required
@require_http_methods(['POST'])
def acc_maintenance_delete(request, pk):
    if not has_permission(request.user, Perms.MAINTENANCE_DELETE):
        return JsonResponse({'success': False, 'message': _('Permission denied.')}, status=403)
    rec = get_object_or_404(AccessoryMaintenanceRecord, pk=pk)
    if rec.is_open:
